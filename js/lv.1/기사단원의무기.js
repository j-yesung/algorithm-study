...
// 만약, 이웃나라와의 협약으로 정해진 공격력의 제한수치가 3이고 제한수치를 초과한 기사가 사용할 무기의 공격력이 2라면, 15번으로 지정된
// 기사단원은 무기점에서 공격력이 2인 무기를 구매합니다. 무기를 만들 때, 무기의 공격력 1당 1kg의 철이 필요합니다.
...
function solution(number, limit, power) {
  const numArr = Array.from(Array(number), (_, index) => index + 1);
  const divArr = [];

  for (let i = 0; i < numArr.length; i++) {
    let divNo = findDivisor(numArr[i]);

    if (divNo > limit) divNo = power;
    divArr.push(divNo);
  }

  return divArr.reduce((acc, cur) => acc + cur, 0);
}
